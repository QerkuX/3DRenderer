...
def main(graphSize):
    ...
    changeOffset(graphSize, vertices)
...
def changeOffset(graphSize, vertices):
    ...
    while True:
        for xoffset in range(60, 121, 1):
            oldVertices = render(graphSize, vertices, oldVertices, xoffset, 60)

        for yoffset in range(60, 121, 1):
            oldVertices = render(graphSize, vertices, oldVertices, 120, yoffset)

        for yoffset in range(120, 59, -1):
            oldVertices = render(graphSize, vertices, oldVertices, 120, yoffset)

        for xoffset in range(120, 59, -1):
            oldVertices = render(graphSize, vertices, oldVertices, xoffset, 60)

        for yoffset in range(60, 121, 1):
            oldVertices = render(graphSize, vertices, oldVertices, 60, yoffset)
